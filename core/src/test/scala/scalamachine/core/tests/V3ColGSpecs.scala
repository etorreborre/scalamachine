...
class V3ColGSpecs extends Specification with Mockito with SpecsHelper with WebmachineDecisions { def is = s2""" ${ "Webmachine V3 Column G".title }
  ...
  G7 - Resource Exists?                                                         
    Sets the Vary header after conneg                                           
      ...
      if resource returns non-empty list, those values are additional             $testVaryResourceAdditional 
